...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  first_conv = tf.nn.conv2d(fingerprint_4d, first_weights, [1, 1, 1, 1],
                            'SAME') + first_bias
  first_elu = tf.nn.elu(first_conv)
  if is_training:
    first_dropout = tf.nn.dropout(first_elu, dropout_prob)
  else:
    first_dropout = first_elu
  ...
  second_conv = tf.nn.conv2d(max_pool, second_weights, [1, 1, 1, 1],
                             'SAME') + second_bias
  second_elu = tf.nn.elu(second_conv)
  if is_training:
    second_dropout = tf.nn.dropout(second_elu, dropout_prob)
  else:
    second_dropout = second_elu
